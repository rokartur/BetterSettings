...
### The "Show Details" toggle animation (the most intricate piece)
This is one synchronized animation across *every visible row* in the active tab, orchestrated by the singleton `SettingsDetailsAnimationCoordinator` and driven by a `betterSettingsShowDetailsDidChange` notification posted in two phases:
1. **`subtitleFade`** — fade subtitles out (no-op when revealing).
2. **`layoutTransition`** — animate section heights inside one `NSAnimationContext` group, with **scroll-position compensation** (`captureScrollAnchor` before, `animateScrollCompensation` after `flushLayouts`) so content above the fold changing height doesn't visually jump the viewport.

A monotonic **generation counter** (`beginToggleAnimation` → `activeToggleGeneration`) invalidates stale in-flight animations; rows register layout roots + completions against the current generation. The coordinator only animates rows in the active tab (`isViewInActiveTab`); above-the-fold height changes are absorbed by the scroll-position compensation rather than by throttling rows.

`SettingsRowView` owns the per-row half of this: it caches its expanded subtitle height (invalidate on text/width change), drives height/alpha/transform animations with their own per-row generation counter, can re-sync from the presentation layer mid-flight if interrupted, and applies the immediate (un-animated) state for inactive tabs and reduce-motion.
