...
`SettingsDetailsSpringAnimator` is the whole engine: it coalesces all rows that register on the same run-loop turn under **one `CADisplayLink`** (60 Hz `Timer` below macOS 14). Each row integrates its **own** semi-implicit-Euler spring whose `response` scales with how far that row travels — a 1-line subtitle snaps, a tall multi-line one glides (short rows looked floaty on a fixed duration) — with per-direction damping (expand under-damped → overshoots the open height; collapse more damped → a smooth close, since its past-0 overshoot is clamped and can't bounce visibly). Rows also **cascade** top-to-bottom via a small per-row start delay so identical-height rows don't move in dead unison. In one pass per tick it advances every track's height + opacity, lays out each affected root once, and re-pins a captured **scroll anchor** so content above the fold changing height doesn't drift the viewport. Each row finishes (snaps to rest) as its own spring settles. Tuning constants — `response{Expand,Collapse}{Short,Long}`, `damping{Expand,Collapse}`, `referenceTravel`, `stagger` — live at the top of the file. `cancel(_:)` drops a row's track when it takes over its own state (e.g. its subtitle text changed).
